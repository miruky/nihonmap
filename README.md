...
同梱のデモ(`src/`)はライブラリの実利用例で、人口・人口密度・地方区分の塗り分け、県名・読み・コードでの検索、表示中の地図のSVG書き出し、ライト/ダーク/システムのテーマ切替を備える。モード・塗り分け・沖縄別枠の状態はURLに保存され、リンクを開けば同じ表示が再現される。
...
| デモ                 | Vite 8                                                  |
| テスト               | Vitest 4 + happy-dom(DOMが要るテストのみ)               |
...
- `src/demo/` — デモ専用のデータと機能
  - `population.ts` / `area.ts` — 2020年国勢調査の人口と国土地理院の面積
  - `search.ts` — 県名・読み・コードの絞り込み
  - `export.ts` — 表示中の地図を単体SVGへ書き出す
- `src/theme.ts` — ライト/ダーク/システムのテーマ解決
...
コードは [MIT](LICENSE)。境界データの出典はNatural Earth(パブリックドメイン)、デモの人口は総務省統計局「令和2年国勢調査」、面積は国土地理院「全国都道府県市区町村別面積調」による。
